...
[@b.form action="!save" title="申报基本信息" theme="list" enctype="multipart/form-data"]
  [@b.textarea name="assessApply.activitySummary" id="assessApply_activitySummary" label="创建活动及其效果" value="${assessApply.activitySummary!}" required="true"  cols="100" rows="5" maxlength="2000" comment="300字以内" editor="kind"/]
  [@b.textarea name="assessApply.wenmingSummary" id="assessApply_wenmingSummary" label="文明创建特色" value="${assessApply.wenmingSummary!}" required="true"  cols="100" rows="5" maxlength="2000" comment="300字以内" editor="kind"/]
