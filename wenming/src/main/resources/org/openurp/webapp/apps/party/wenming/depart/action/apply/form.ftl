...
[@b.form action="!save" title="申报基本信息" theme="list" enctype="multipart/form-data"]
  [@b.textarea id="assessApply_activitySummary"  name="assessApply.activitySummary" label="创建活动及其效果概要" value="${assessApply.activitySummary!}" required="true"  cols="100" rows="5" maxlength="500" comment="300字以内"/]
  [@b.textarea id="assessApply_wenmingSummary"  name="assessApply.wenmingSummary" label="文明创建特色工作概要" value="${assessApply.wenmingSummary!}" required="true"  cols="100" rows="5" maxlength="500" comment="300字以内"/]
...
[/@]
<script>
   /*KindEditor.create('#assessApply_activitySummary', {
            themeType : 'simple'
        });
   KindEditor.create('#assessApply_wenmingSummary');
   */
</script>
[@b.foot/]